...
- [x] S3 — Odometer AI reading — built 2026-07-18, tested on-device 2026-07-19 (mobile-v2 only: `/api/ai/extract-odometer` route + BYOK direct path + `TripsScreen.tsx` origin/destination wiring, `ai_odometer_scan` gate). Two bugs found and fixed during testing: (1) evidence photo preview showed the raw `file://` URI as text instead of an actual thumbnail — `EvidenceCapture` now renders an `Image` when the value looks like a real file/http URI; (2) the "Save Car/Motorcycle Trip" button silently stayed disabled with no explanation whenever `destinationReading` wasn't greater than `originReading` (the real gate all along — `odoValid` requires a positive distance, not the optional location text fields) — added an inline warning showing both readings so the tester can see why. Web page wiring deliberately skipped — `apps/user` pages aren't the real product, same call as S1.
