...
- [~] S4 — Voice claim entry (cloud) — built 2026-07-18, mobile-v2 only (standalone voice-entry button, `/api/ai/parse-voice-claim`, `ai_voice_claim` gate, reusable `VoiceRecorderModal`). Needs `npx expo install expo-audio` before it compiles — new dependency approved but not installed in this session, and its exact API surface for this Expo SDK wasn't verified against real types. Web wiring deliberately skipped, same as S3. Not yet manually tested by Eff.
