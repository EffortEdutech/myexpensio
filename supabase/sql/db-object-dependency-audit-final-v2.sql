-- DB object dependency audit (final, safe version v2)
...
-- -----------------------------------------------------------------------------
-- BLOCK 3: normal function bodies containing legacy names
-- -----------------------------------------------------------------------------
select
  n.nspname as schema_name,
...
where lower(coalesce(qual, '')) like '%agents%'
   ...
   or lower(coalesce(qual, '')) like '%v_partner_commission_summary%'
   ...
   or lower(coalesce(with_check, '')) like '%v_partner_commission_summary%'
order by schemaname, tablename, policyname;

-- -----------------------------------------------------------------------------
-- BLOCK 5: triggers backed by normal functions containing legacy names
-- -----------------------------------------------------------------------------
select distinct
