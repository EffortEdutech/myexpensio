...
//  ── OR ──
// apps/admin/lib/export-columns.ts  (copy to apps/user/lib/ as well)
//
// CANONICAL column registry for the export template system.
// This file is the single source of truth for:
//   1. Every column available in an export
//   2. Which preset includes each column (STANDARD vs COMPLETE)
//   3. The human-readable label shown in the TemplateEditor
//
// Both Admin App (template editor) and User App (export-builder) import this.
...
export type ColumnDef = {
  key: ExportColumnKey
  label: string            // Column header in the exported file
  description: string      // Shown in template editor tooltip
  presets: ColumnPreset[]  // Which presets include this column by default
  group: string            // UI grouping in template editor
}
...
export const ALL_COLUMNS: ColumnDef[] = [
  ...
  { key: 'user_email',         label: 'Staff Email',      description: 'Email of the claimant',                   presets: ['COMPLETE'],            group: 'Claim' },
  ...
  { key: 'submitted_at',       label: 'Submitted Date',   description: 'Date & time claim was submitted',         presets: ['STANDARD','COMPLETE'], group: 'Claim' },
  { key: 'total_amount',       label: 'Claim Total (MYR)',description: 'Total reimbursement amount for the claim', presets: ['STANDARD','COMPLETE'], group: 'Claim' },

  // Item-level core
  { key: 'item_id',            label: 'Item ID',          description: 'Unique item identifier (UUID)',            presets: ['COMPLETE'],            group: 'Item' },
  { key: 'item_type',          label: 'Item Type',        description: 'e.g. MILEAGE, MEAL, TOLL, PER_DIEM',      presets: ['STANDARD','COMPLETE'], group: 'Item' },
  { key: 'item_date',          label: 'Item Date',        description: 'Date the expense was incurred',           presets: ['STANDARD','COMPLETE'], group: 'Item' },
  { key: 'amount',             label: 'Amount (MYR)',     description: 'Reimbursement amount for this item',      presets: ['STANDARD','COMPLETE'], group: 'Item' },
  { key: 'currency',           label: 'Currency',         description: 'Always MYR in Phase 1',                  presets: ['COMPLETE'],            group: 'Item' },
  { key: 'merchant',           label: 'Merchant / Vendor',description: 'Merchant name or description',           presets: ['COMPLETE'],            group: 'Item' },
  { key: 'notes',              label: 'Notes',            description: 'Additional notes entered by staff',       presets: ['COMPLETE'],            group: 'Item' },
  { key: 'receipt_present',    label: 'Receipt (Y/N)',    description: 'Whether a receipt was attached',          presets: ['STANDARD','COMPLETE'], group: 'Item' },
  { key: 'receipt_url',        label: 'Receipt URL',      description: 'Direct URL to uploaded receipt',         presets: ['COMPLETE'],            group: 'Item' },

  // Mileage / trip
  { key: 'trip_origin',        label: 'Origin',           description: 'Trip starting point text',               presets: ['COMPLETE'],            group: 'Mileage' },
  { key: 'trip_destination',   label: 'Destination',      description: 'Trip ending point text',                 presets: ['COMPLETE'],            group: 'Mileage' },
  { key: 'distance_km',        label: 'Distance (KM)',    description: 'Final distance in KM (from meters)',     presets: ['COMPLETE'],            group: 'Mileage' },
  { key: 'distance_source',    label: 'Distance Source',  description: 'GPS / SELECTED_ROUTE / ODOMETER_OVERRIDE', presets: ['COMPLETE'],          group: 'Mileage' },
  { key: 'mileage_rate',       label: 'Mileage Rate (MYR/km)', description: 'Rate used at time of claim',        presets: ['COMPLETE'],            group: 'Mileage' },
...
// Used by export-builder.ts in the user app.
export function resolveColumns(schema: {
  preset?: ColumnPreset
  columns?: ExportColumnKey[]
}): ExportColumnKey[] {
  // If custom columns array is set and non-empty, use it
  if (schema.columns && schema.columns.length > 0) {
    // Filter to only valid keys (safety)
