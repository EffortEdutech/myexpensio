...
// Production user app URL — override with NEXT_PUBLIC_USER_APP_URL env var if needed
const USER_APP_URL = process.env.NEXT_PUBLIC_USER_APP_URL ?? 'https://myexpensio-jade.vercel.app'
...
export async function middleware(request: NextRequest) {
  ...
  if (isPublic) {
    if (user && pathname === '/login') {
      ...
      if (profile?.role === 'USER') {
        ...
        // Employee trying to access /login on admin app → send to user app
        return NextResponse.redirect(new URL(USER_APP_URL))
      }
    }
    return response
  }
  ...
  if (platformRole === 'USER') {
    ...
    // → redirect cleanly to the user app
    return NextResponse.redirect(new URL(USER_APP_URL))
  }
