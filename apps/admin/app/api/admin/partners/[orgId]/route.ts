...
export async function PATCH(
  ...
  // Confirm the org exists and is a partner
  ...
  if (!existing.is_partner) return err('VALIDATION_ERROR', 'Organisation is not a partner.', 400)

  const allowed = ['ACTIVE', 'SUSPENDED', 'TERMINATED']
  if (body.partner_status && !allowed.includes(body.partner_status)) {
    return err('VALIDATION_ERROR', `partner_status must be one of: ${allowed.join(', ')}`, 400)
  }
