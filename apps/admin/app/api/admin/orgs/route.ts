...
export async function POST(req: Request) {
  ...
  // Step 2: Create owner with temp-password onboarding
  ...
  if (!userExisted && tempPassword) {
    try {
      ...
      console.error('[admin/orgs] owner onboarding email failed:', mailErr)
  ...
  await db.from('audit_logs').insert({
    ...
    metadata:      {
      ...
      initial_tier, user_existed: userExisted, email_sent: emailSent,
      provisioned_by: ctx.email,
    },
  })

  return NextResponse.json(
    { org_id: orgId, org_name: org.name, workspace_type, user_id: userId, user_existed: userExisted, email_sent: emailSent },
    { status: 201 },
  )
}
