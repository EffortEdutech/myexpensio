...
type Claim = {
  id: string; user_id: string; status: 'DRAFT' | 'SUBMITTED'
  title: string | null; total_amount: number | null; currency: string
  period_start: string | null; period_end: string | null
  submitted_at: string | null; created_at: string; profiles: Profile | null
}
...
function initials(p: Profile | null) { return (p?.display_name ?? p?.email ?? '?').slice(0, 2).toUpperCase() }

function StatusBadge({ status }: { status: string }) {
  return status === 'SUBMITTED' ? (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-green-50 text-green-700">
      <span className="w-1.5 h-1.5 rounded-full bg-green-500" />Submitted</span>
  ) : (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-amber-50 text-amber-700">
      <span className="w-1.5 h-1.5 rounded-full bg-amber-400" />Draft</span>
  )
}

function KPICard({ label, value, sub, color = 'gray' }: {
  label: string; value: string | number; sub?: string; color?: 'gray' | 'green' | 'amber' | 'blue'
}) {
...
function ClaimsList({ orgId, orgName, onChangeWorkspace }: {
  ...
  const totalAmount    = claims.filter(c => c.status === 'SUBMITTED').reduce((s, c) => s + (c.total_amount ?? 0), 0)
  ...
  const fetchClaims = useCallback(async (p: number) => {
    ...
    try {
      ...
      setClaims(json.claims ?? []); setTotal(json.total ?? 0)
    } catch { /* silent */ }
    finally { setLoading(false) }
  }, [orgId, status, dateFrom, dateTo])
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {orgName ?? 'Workspace'} — read-only view
        ...
        <Link href="/exports" className="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 font-medium hover:bg-gray-50 flex-shrink-0">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <KPICard label="Total claims"    value={total}          sub="in workspace" />
        <KPICard label="Submitted"       value={submittedCount} sub="locked" color="green" />
      ...
      <div className="flex flex-wrap gap-3 items-end">
        <select value={status} onChange={e => setStatus(e.target.value)}
          ...
          <option value="SUBMITTED">Submitted</option><option value="DRAFT">Draft</option>
        </select>
        ...
        {(status || dateFrom || dateTo) && <button onClick={() => { setStatus(''); setDateFrom(''); setDateTo('') }} className="text-xs text-gray-400 hover:text-gray-600">Clear</button>}
                  ...
                  <tr key={claim.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <div className="text-sm font-medium text-gray-900">{claim.profiles?.display_name ?? '—'}</div>
                          <div className="text-xs text-gray-400">{claim.profiles?.email ?? '—'}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-900 max-w-40 truncate">{claim.title || <span className="text-gray-400 italic">Untitled</span>}</td>
                    <td className="px-4 py-3 text-xs text-gray-500 whitespace-nowrap">{fmtPeriod(claim.period_start, claim.period_end)}</td>
                    <td className="px-4 py-3"><StatusBadge status={claim.status} /></td>
                    <td className="px-4 py-3 text-sm font-medium text-gray-900 tabular-nums">{fmtMoney(claim.total_amount, claim.currency)}</td>
                    <td className="px-4 py-3 text-xs text-gray-400">{fmtDate(claim.submitted_at)}</td>
                    <td className="px-4 py-3">
                      <Link href={`/claims/${claim.id}`} className="text-xs text-blue-600 hover:text-blue-800 font-medium">View</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex gap-2">
            {[{ label: 'Previous', p: page - 1, disabled: page === 1 },
              ...
              <button key={label} onClick={() => { setPage(p); fetchClaims(p) }} disabled={disabled}
...
export default function ClaimsPage() {
  ...
  if (showPicker) {
    return (
      <div className="space-y-5">
        <div><h1 className="text-lg font-semibold text-gray-900">Claims Oversight</h1>
          <p className="text-sm text-gray-500 mt-0.5">Select a workspace to view its claims</p></div>
        <div className="bg-white rounded-xl border border-gray-200 py-8">
          <InternalOrgPicker label="claims" onSelect={(id, name) => { setSelectedOrg({ id, name }); setShowPicker(false) }} />
        </div>
      </div>
    )
  }
