...
//
// Export Job History — workspace-scoped.
// Shows all export jobs generated by any user in this workspace.
// Accessible by OWNER, ADMIN, MANAGER.
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${c.cls}`}>
      {c.label}
    </span>
  )
}
...
function FormatBadge({ format }: { format: string }) {
  ...
  return (
    <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${cls[format] ?? 'bg-gray-100 text-gray-600'}`}>
      {format}
    </span>
  )
}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            All export jobs generated by members of this workspace.
          </p>
        </div>
        <Link href="/claims"
      ...
      <div className="flex flex-wrap gap-3 items-end">
        ...
        {(format || status) && (
          <button onClick={() => { setFormat(''); setStatus('') }}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {jobs.map(job => (
                  <tr key={job.id}
                    className={`border-b border-gray-50 hover:bg-gray-50 transition-colors ${job.status === 'FAILED' ? 'bg-red-50/20' : ''}`}>
                    ...
                    <td className="px-4 py-3">
                      ...
                      {job.status === 'FAILED' && job.error_message && (
                        <p className="text-xs text-red-500 mt-0.5 max-w-32 truncate" title={job.error_message}>
                          {job.error_message}
                        </p>
                      )}
                    </td>
