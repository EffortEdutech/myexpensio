'use client'
// apps/user/app/(app)/business/income/page.tsx
// List income entries for the BUSINESS space, filtered by month.
...
export default function IncomeListPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/ledger?spaceId=${spaceId}&month=${month}&year=${year}&type=INCOME&limit=100`)
      .then(r => r.json())
      .then(d => {
      ...
      .catch(() => setError('Failed to load income entries.'))
    ...
    <div style={S.page}>
      ...
      <div style={S.summaryCard}>
        ...
        <div style={S.summaryRight}>
          ...
          <Link href="/business/add-income" style={S.addBtn}>+ Add</Link>
        </div>
      </div>
      ...
      <div style={S.listCard}>
        ...
        {!loading && entries.length === 0 && (
          <div style={S.empty}>No income recorded for {MONTHS[month - 1]} {year}.<br />
            <Link href="/business/add-income" style={S.emptyLink}>Add your first income entry →</Link>
          </div>
        )}
        {entries.map((entry, i) => (
          <div key={entry.id} style={{ ...S.entryRow, borderTop: i === 0 ? 'none' : '1px solid #f8fafc' }}>
            <div style={S.entryLeft}>
              ...
              {entry.income_source && (
                <div style={S.sourceTag}>{SOURCE_LABELS[entry.income_source] ?? entry.income_source}</div>
              )}
...
const S: Record<string, React.CSSProperties> = {
  ...
  summaryAmount: { fontSize: 24, fontWeight: 800, color: '#16a34a' },
  summaryRight:  { display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 },
  addBtn:        { display: 'inline-block', padding: '8px 16px', borderRadius: 10, backgroundColor: '#16a34a', color: '#fff', fontWeight: 700, fontSize: 13, textDecoration: 'none' },
  ...
  emptyLink:     { color: '#16a34a', fontWeight: 600, textDecoration: 'none' },
  ...
  sourceTag:     { fontSize: 11, color: '#16a34a', backgroundColor: '#f0fdf4', borderRadius: 6, padding: '2px 7px', width: 'fit-content' },
  ...
  entryAmount:   { fontSize: 15, fontWeight: 700, color: '#16a34a' },
