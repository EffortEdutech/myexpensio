...
function ExpenseModal({ type, onAdd, onClose, editMode = false, itemId, initialData }: {
  type: 'MEAL' | 'LODGING'
  ...
  useEffect(() => {
    fetch('/api/settings/rates')
      ...
      .then(j => {
        if (j.rate) {
          setRates({
            morning: Number(j.rate.meal_rate_morning  ?? j.rate.meal_rate_per_session ?? 20),
            noon:    Number(j.rate.meal_rate_noon     ?? j.rate.meal_rate_per_session ?? 30),
            evening: Number(j.rate.meal_rate_evening  ?? j.rate.meal_rate_per_session ?? 30),
    ...
    >
      ...
      {type === 'MEAL' && mode === 'FIXED_RATE' && (
        <div style={S.field}>
          ...
          {SESSION_META.map(s => (
            <button key={s.key} onClick={() => tickSession(s.key)} disabled={fullDay} style={{
              ...S.sessionBtn,
              borderColor: sessions.has(s.key) && !fullDay ? '#0f172a' : '#e2e8f0',
              backgroundColor: sessions.has(s.key) && !fullDay ? '#f8fafc' : '#fff',
              opacity: fullDay ? 0.4 : 1, marginBottom: 4,
            }}>
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.section}>
        ...
        {items.length === 0
          ? <div style={{ padding: '24px 16px', fontSize: 13, color: '#94a3b8', textAlign: 'center' }}>
              No items yet. Add mileage, meal, lodging, toll, parking, or transport.
            </div>
      ...
      {modal === 'EDIT_MEAL' && editingItem && (
        <ExpenseModal type="MEAL" editMode itemId={editingItem.id} initialData={editingItem}
          onAdd={d => updateExpense(editingItem.id, 'MEAL', d)}
          onClose={() => { setModal(null); setEditingItem(null) }}
        />
      )}
      {modal === 'EDIT_LODGING' && editingItem && (
        <ExpenseModal type="LODGING" editMode itemId={editingItem.id} initialData={editingItem}
...
const S: Record<string, React.CSSProperties> = {
  ...
  input:      { paddingTop: 10, paddingBottom: 10, paddingLeft: 12, paddingRight: 12, border: '1.5px solid #e2e8f0', borderRadius: 8, fontSize: 14, color: '#0f172a', backgroundColor: '#fff', outline: 'none', boxSizing: 'border-box', WebkitTextFillColor: '#0f172a' },
  ...
  sessionBtn: { display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', border: '1.5px solid #e2e8f0', borderRadius: 10, backgroundColor: '#fff', cursor: 'pointer', textAlign: 'left', width: '100%', boxSizing: 'border-box' },
}
