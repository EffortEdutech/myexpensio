...
//
// RECONCILED — 10 Mar 2026
//   Base:   fec334c  (TollParkingModal + TransportModal + TNG pending badge)
//   Patch 1: Rates endpoint → /api/settings/rates  (ae99090 canonical)
//   Patch 2: ReceiptUploader → storagePath + enableScan props (ae99090 canonical)
//   Discard: 783c700 entirely
// ─────────────────────────────────────────────────────────────────────────────
...
function ItemCard({
  ...
  const isTngVerified =
    (item.type === "TOLL" || item.type === "PARKING") &&
    !!item.tng_transaction_id &&
  ...
  function sub() {
    ...
    if (["TAXI", "GRAB", "TRAIN", "FLIGHT", "BUS"].includes(item.type))
      return (
        item.merchant || item.type.charAt(0) + item.type.slice(1).toLowerCase()
      );
    if (item.type === "PER_DIEM") {
      const days = Number(item.perdiem_days ?? item.qty ?? 0);
      const rate = Number(item.perdiem_rate_myr ?? item.rate ?? 0);
      const parts: string[] = [];
      if (days > 0) parts.push(`${days} day${days !== 1 ? "s" : ""}`);
      if (rate > 0) parts.push(`MYR ${rate.toFixed(2)}/day`);
      if (item.perdiem_destination) parts.push(item.perdiem_destination);
      return parts.join(" · ") || "Per diem allowance";
    }
    const parts: string[] = [];
    if (item.meal_session)
      parts.push(SESSION_LABEL[item.meal_session] ?? item.meal_session);
    if (item.type === "LODGING" && item.qty)
      parts.push(`${item.qty} night${item.qty !== 1 ? "s" : ""}`);
  ...
  const canUnlinkTng =
    ...
    !!item.tng_transaction_id;
  const unlinkBusy =
    !!item.tng_transaction_id && unlinkingTngId === item.tng_transaction_id;
            ...
            {canUnlinkTng && (
              <button
                ...
                style={{
                  ...
                  borderRadius: 999,
                  color: "#dc2626",
                  cursor: unlinkBusy ? "not-allowed" : "pointer",
                  fontSize: 10,
                  fontWeight: 700,
                  opacity: unlinkBusy ? 0.65 : 1,
                }}
              >
...
function MileageModal({
  ...
  useEffect(() => {
    fetch("/api/trips?status=FINAL")
      ...
      .then((j) => {
        setTrips(
          (j.items ?? []).filter(
            (t: Trip) => !alreadyAddedTripIds.includes(t.id),
          ),
        );
        setLoading(false);
      })
  ...
  const footer = (
    <button
      ...
      style={{
        ...S.btnModalAdd,
        opacity: saving || selected.size === 0 ? 0.45 : 1,
      }}
    >
      {saving
        ? "Adding…"
        : `Add ${selected.size > 0 ? selected.size + " " : ""}Trip${selected.size !== 1 ? "s" : ""}`}
    </button>
  );
    ...
    <Modal title="Add Mileage" onClose={onClose} footer={footer}>
      {loading ? (
        ...
        <div style={S.mInfo}>
          ...
          <br />
          Complete a trip first.
        </div>
      ) : (
        <>
          <p style={{ fontSize: 12, color: "#64748b", margin: 0 }}>
            Select one or more completed trips:
          </p>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      {t.origin_text && t.destination_text
                        ? `${t.origin_text} → ${t.destination_text}`
                        : t.calculation_mode === "GPS_TRACKING"
                          ? "GPS Trip"
                          : "Planned Trip"}
                    </div>
                    <span style={{ fontSize: 11, color: "#94a3b8" }}>
                      {fmtKm(t.final_distance_m)} · {fmtDate(t.started_at)}
                    </span>
                  </div>
                  <span
                    style={{
...
// PATCH: Rates fetched from /api/settings/rates (ae99090 canonical endpoint).
// PATCH: ReceiptUploader uses storagePath + enableScan props (ae99090 camera API).
...
function ExpenseModal({
  ...
  const initSessions =
    initSession && !initFullDay && initSession !== ""
      ? new Set<"MORNING" | "NOON" | "EVENING">([
          initSession as "MORNING" | "NOON" | "EVENING",
        ])
      : new Set<"MORNING" | "NOON" | "EVENING">();
  ...
  const [sessions, setSessions] =
    useState<Set<"MORNING" | "NOON" | "EVENING">>(initSessions);
  // PATCH: receiptPath typed as string (ae99090) — seeded from existing item when editing
  const [receiptPath, setReceiptPath] = useState<string>(
    initialData?.receipt_url ?? "",
  );
  const [rates, setRates] = useState<MealRates>({
    morning: 20,
    noon: 30,
    evening: 30,
    fullDay: 60,
    ...
    fetch("/api/settings/rates")
      ...
      .then((j) => {
        if (j.rate) {
          setRates({
            morning: Number(
              j.rate.meal_rate_morning ?? j.rate.meal_rate_per_session ?? 20,
            ),
            noon: Number(
              j.rate.meal_rate_noon ?? j.rate.meal_rate_per_session ?? 30,
            ),
            evening: Number(
              j.rate.meal_rate_evening ?? j.rate.meal_rate_per_session ?? 30,
            ),
  ...
  const mealTotal = fullDay
    ? rates.fullDay
    : [...sessions].reduce(
        (s, k) => s + (rates[k.toLowerCase() as keyof MealRates] ?? 0),
        0,
      );
    ...
    if (type === "LODGING") {
      ...
      try {
        await onAdd({
          ...
          lodging_check_out: checkOut,
          amount:
            mode === "FIXED_RATE"
              ? lodgingRate * nightCount
              : parseFloat(amount),
          rate: mode === "FIXED_RATE" ? lodgingRate : undefined,
          // PATCH: receipt_path key (ae99090 pattern — addExpense strips and re-keys as receipt_url)
          receipt_path:
            mode === "RECEIPT" && receiptPath ? receiptPath : undefined,
    ...
    try {
      if (fullDay) {
        await onAdd({
          mode: "FIXED_RATE",
          claim_date: claimDate,
          meal_session: "FULL_DAY",
          amount: rates.fullDay,
          rate: rates.fullDay,
        });
      } else {
  ...
  const btnLabel = saving
    ...
    : type === "LODGING"
      ? mode === "FIXED_RATE"
        ? `Add Lodging · MYR ${(lodgingRate * nightCount).toFixed(2)}`
        : "Add Lodging"
      : mode === "RECEIPT"
    ...
    >
      ...
      {type === "MEAL" && mode === "FIXED_RATE" && (
        <div style={S.field}>
          ...
          <button
            ...
            style={{
              ...S.sessionBtn,
              borderColor: fullDay ? "#0f172a" : "#e2e8f0",
              backgroundColor: fullDay ? "#f8fafc" : "#fff",
              marginBottom: 4,
            }}
          >
          ...
          {SESSION_META.map((s) => (
            <button
              key={s.key}
              onClick={() => tickSession(s.key)}
              disabled={fullDay}
              style={{
                ...S.sessionBtn,
                borderColor:
                  sessions.has(s.key) && !fullDay ? "#0f172a" : "#e2e8f0",
                backgroundColor:
                  sessions.has(s.key) && !fullDay ? "#f8fafc" : "#fff",
                opacity: fullDay ? 0.4 : 1,
                marginBottom: 4,
              }}
            >
      ...
      {mode === "RECEIPT" && (
        <>
          ...
          <div style={S.field}>
            ...
            {/* PATCH: ae99090 ReceiptUploader API — storagePath + enableScan (camera-enabled) */}
            <ReceiptUploader
      ...
      <div style={S.field}>
        ...
        <input
          ...
          placeholder={
            type === "MEAL" ? "e.g. Nasi Kandar Pelita" : "e.g. Hotel Maya KL"
          }
          style={S.input}
        />
      </div>
...
//         Amount pending until user imports TNG statement.
//   OFF → manual amount + optional receipt scan.
// PATCH: ReceiptUploader uses storagePath + enableScan (ae99090 camera API).

function TollParkingModal({
  type,
  onAdd,
  onClose,
}: {
  ...
  // PATCH: ae99090 ReceiptUploader pattern — typed as string
  const [receiptPath, setReceiptPath] = useState<string>("");
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  async function handleAdd() {
    ...
    try {
      await onAdd({
        ...
        claim_date: claimDate,
        entry_location:
          type === "TOLL" ? entryLocation.trim() || undefined : undefined,
        exit_location:
          type === "TOLL" ? exitLocation.trim() || undefined : undefined,
  ...
  const btnDisabled =
    saving || (!tngMode && (!amount || parseFloat(amount) <= 0));
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          gap: 12,
          padding: "12px 14px",
          border: `2px solid ${tngMode ? "#0f172a" : "#e2e8f0"}`,
          borderRadius: 12,
          backgroundColor: tngMode ? "#f8fafc" : "#fff",
          cursor: "pointer",
          textAlign: "left",
          width: "100%",
        }}
      ...
      {tngMode && (
        ...
        >
          ...
          <div>
            ...
            >
              This item will show as <strong>TNG · Link pending</strong> in your
              claim. Go to <strong>TNG Importer</strong> to upload your
              eStatement and link the matching transaction to fill in the exact
              amount.
            </div>
          </div>
        </div>
...
function TransportModal({
  ...
  const isTngPending = canPayTng && paidViaTng;
  const btnDisabled =
    saving || (!isTngPending && (!amount || parseFloat(amount) <= 0));
  const typeLabel =
    transportType.charAt(0) + transportType.slice(1).toLowerCase();
  const btnLabel = saving
    ? "Adding…"
    : isTngPending
      ? `Add ${typeLabel} · TNG`
  ...
  const merchantPlaceholder =
    transportType === "GRAB"
      ...
      : transportType === "TAXI"
        ? "e.g. Airport → Hotel"
        : transportType === "TRAIN"
          ? "e.g. ETS Ipoh → KL Sentral"
          : transportType === "BUS"
            ? "e.g. Transnasional KL → Ipoh"
            : "e.g. AirAsia KUL → PEN";
          ...
          {TRANSPORT_META.filter((t) => t.type !== "FLIGHT").map((t) => (
            <button
              ...
              style={{
                ...
                fontWeight: 700,
                backgroundColor:
                  transportType === t.type ? "#0f172a" : "#f1f5f9",
      ...
      {isTngPending && (
        ...
        >
          ...
          <div>
            ...
            >
              You can add this transport item without entering an amount. Link
              the matching TNG statement later to fill in the exact amount.
            </div>
          </div>
        </div>
      )}
...
function PerDiemModal({
  ...
  useEffect(() => {
    ...
    fetch("/api/settings/rates")
      ...
      .then((j) => {
        if (!active) return;
        setRate(Number(j.rate?.perdiem_rate_myr ?? 0));
      })
      .catch(() => {
        if (!active) return;
        setRate(0);
  ...
  const total = Number((Math.max(1, dayCount) * Math.max(0, rate)).toFixed(2));
  const rateConfigured = rate > 0;
  ...
  async function handleAdd() {
    ...
    if (!rateConfigured) {
      setError(
        "Per diem rate is not configured. Set it first in Settings → Rates.",
      );
      return;
    }
    ...
    try {
      await onAdd({
        type: "PER_DIEM",
        claim_date: claimDate,
        perdiem_days: dayCount,
        perdiem_rate_myr: rate,
        perdiem_destination: destination.trim() || undefined,
        notes: notes.trim() || undefined,
      });
    } catch (e: unknown) {
      setError((e as Error).message);
  ...
  const btnDisabled = saving || loadingRate || !rateConfigured || dayCount <= 0;
  const btnLabel = saving
    ? "Adding…"
    : rateConfigured
      ? `Add Per Diem · MYR ${total.toFixed(2)}`
      : "Add Per Diem";

  return (
    <Modal
      title="Add Per Diem Allowance"
      onClose={onClose}
      footer={
        <button onClick={handleAdd} disabled={btnDisabled} style={S.btnModalAdd}>
          {btnLabel}
        </button>
      }
    >
      <div style={{ display: "grid", gap: 12 }}>
        ...
        <div
          style={{
            padding: "12px 14px",
            borderRadius: 12,
            border: `1px solid ${rateConfigured ? "#bbf7d0" : "#fecaca"}`,
            backgroundColor: rateConfigured ? "#f0fdf4" : "#fef2f2",
          }}
        >
          ...
          <div style={{ fontSize: 13, marginTop: 4, color: "#0f172a" }}>
            {loadingRate
              ? "Loading rate…"
              : rateConfigured
                ? `MYR ${rate.toFixed(2)} per day`
                : "Not configured yet"}
          </div>
          <div style={{ fontSize: 11, color: "#64748b", marginTop: 4 }}>
            {loadingRate
              ? "Fetching your current default rate from Settings."
              : rateConfigured
                ? `Estimated total: MYR ${total.toFixed(2)}`
                : "Go to Settings → Rates and set Per Diem Rate first."}
          </div>
        </div>

        <div>
          <label style={S.label}>Destination (optional)</label>
          <input
            value={destination}
        ...
        <div>
          ...
          <input
            value={notes}
            onChange={(e) => setNotes(e.target.value)}
            placeholder="e.g. Outstation assignment"
            style={S.input}
          />
        </div>
...
export default function ClaimDetailPage() {
  ...
  const [sortMode, setSortMode] = useState<"DATE_TYPE" | "TYPE_DATE">(
    "DATE_TYPE",
  );
  const [deleting, setDeleting] = useState(false);
  ...
  async function addPerDiemItem(data: Record<string, unknown>) {
    const res = await fetch(`/api/claims/${id}/items`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
  ...
  async function unlinkTng(item: ClaimItem) {
    ...
    try {
      const res = await fetch(`/api/transactions/${tngId}/link`, {
        method: "DELETE",
      });
      ...
      if (!res.ok) {
        setTngActionErr(
          json.error?.message ?? "Failed to unlink TNG transaction.",
        );
        return;
      }
  ...
  async function handleSubmit() {
    ...
    const json = await res.json();
    if (!res.ok) {
      setSubmitErr(json.error?.message ?? "Failed to submit.");
      setSubmitting(false);
      return;
    }
    await load();
  ...
  const title =
    claim.title || periodLabel(claim.period_start, claim.period_end);
  ...
  const tngPendingCount = items.filter(
    (i) => i.mode === "TNG" && Number(i.amount) === 0,
  ).length;
  ...
  const showTngBanner =
    isDraft && (tngPendingCount > 0 || unlinkedTollParkingCount > 0);
  ...
  return (
    <div style={S.page}>
              ...
              .map((item) => (
                <ItemCard
                  ...
                  onEdit={(item) => {
                    setEditingItem(item);
                    setModal(
                      item.type === "MEAL" ? "EDIT_MEAL" : "EDIT_LODGING",
                    );
                  }}
      ...
      >
        ...
        >
          <span style={{ fontSize: 14, fontWeight: 600, color: "#64748b" }}>
            {isDraft && tngPendingCount > 0 ? "Confirmed Total" : "Total"}
          </span>
        ...
        {isDraft && tngPendingCount > 0 && (
          ...
          >
            <span style={{ fontSize: 12, color: "#854d0e", fontWeight: 600 }}>
              💳 + {tngPendingCount} TNG item{tngPendingCount > 1 ? "s" : ""}{" "}
              (amount pending)
            </span>
          </div>
        )}
      ...
      {isDraft && (
        ...
        >
          <button onClick={() => setModal("MILEAGE")} style={S.btnAdd}>
            🚗 Mileage
          </button>
          <button onClick={() => setModal("TOLL")} style={S.btnAdd}>
            🛣️ Toll
          </button>
          <button onClick={() => setModal("PARKING")} style={S.btnAdd}>
            🅿️ Parking
          </button>
          <button onClick={() => setModal("TRANSPORT")} style={S.btnAdd}>
            🚕 Transport
          </button>          
          <button onClick={() => setModal("MEAL")} style={S.btnAdd}>
            🍽 Meal
          </button>
          <button onClick={() => setModal("LODGING")} style={S.btnAdd}>
            🏨 Lodging
          </button>
      ...
      {isDraft && (
        <div>
          {submitErr && (
            <div style={{ ...S.errorBox, marginBottom: 8 }}>{submitErr}</div>
          )}
          {tngPendingCount > 0 && (
            <div
              style={{
            ...
            >
              ⚠️ You have {tngPendingCount} TNG item
              {tngPendingCount > 1 ? "s" : ""} with amount not yet filled. Link
          ...
          <button
            ...
            style={{
              ...S.btnSubmit,
              opacity: submitting || items.length === 0 ? 0.5 : 1,
              cursor:
                submitting || items.length === 0 ? "not-allowed" : "pointer",
            }}
          >
      ...
      {isDraft && (
        <div style={{ marginTop: 4 }}>
          {deleteErr && (
            <div style={{ ...S.errorBox, marginBottom: 8 }}>{deleteErr}</div>
          )}
          {!showDeleteConfirm ? (
            <button
              ...
              <div style={S.confirmRow}>
                <button
                  onClick={async () => {
                    ...
                    setDeleting(false);
                    if (!res.ok) {
                      setDeleteErr(json.error?.message ?? "Failed to delete.");
                      return;
                    }
                    router.push("/claims");
                  }}
      ...
      {modal === "LODGING" && (
        <ExpenseModal
          type="LODGING"
          onAdd={(d) => addExpense("LODGING", d)}
          onClose={() => setModal(null)}
        />
      )}
      {modal === "PER_DIEM" && (
        <PerDiemModal onAdd={addPerDiemItem} onClose={() => setModal(null)} />
      )}
