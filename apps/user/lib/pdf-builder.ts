...
export type PdfData = {
  ...
  claimer_department: string
  claimer_location: string
  claimer_company_name: string
  generated_at: string
...
function getDimensions(orientation: 'portrait' | 'landscape'): PageDimensions {
  if (orientation === 'landscape') return { PW: 841.89, PH: 595.28, CW: 741.89 }
...
export async function buildPdfData(
  ...
  if (itemsWithReceipt.length > 0) {
    const { data: itemRows } = await supabase.from('claim_items').select('id, receipt_url').in('id', itemsWithReceipt)
    for (const row of itemRows ?? []) if (row.receipt_url) receiptPaths[row.id] = row.receipt_url
  }

  const { data: profile } = await supabase
    .from('profiles')
    .select('display_name, email, department, location, company_name')
    .eq('id', userId)
    .single()
  ...
  for (const claim of claimMap.values()) claim.items.sort((a, b) => a.date_iso.localeCompare(b.date_iso))
  ...
  if (tollParkingItemIds.length > 0) {
    const { data: linkRows } = await supabase.from('claim_items').select('id, tng_transaction_id').in('id', tollParkingItemIds)
    const itemToTngId: Record<string, string> = {}
    for (const r of linkRows ?? []) if (r.tng_transaction_id) itemToTngId[r.id] = r.tng_transaction_id

    const tngIds = Object.values(itemToTngId)
    if (tngIds.length > 0) {
      const { data: tngRows } = await supabase.from('tng_transactions').select('id, trans_no, source_file_url, amount').in('id', tngIds)
      ...
      for (const item of allItems) {
        ...
        if (tng.trans_no) {
          ...
          stmtMap.get(key)!.items.push({ trans_no: tng.trans_no, amount: tng.amount != null ? Number(tng.amount).toFixed(2) : '' })
        }
      }
...
async function generatePdfKitBuffer(supabase: SupabaseClient, data: PdfData, signatureDataUrl: string | null): Promise<Buffer> {
...
async function buildDoc(doc: PDFKit.PDFDocument, data: PdfData, signatureDataUrl: string | null, supabase: SupabaseClient) {
  const dim = getDimensions(data.layout.orientation)
  doc.addPage()
  let y = MT
  y = drawHeader(doc, y, dim, data.layout.accent_color)
  y = drawClaimantInfo(doc, y, dim, data)
  if (data.layout.show_summary_table) y = drawSummaryTable(doc, y, dim, data.claims, data.layout.accent_color)
  y = data.layout.grouping === 'BY_CATEGORY'
    ? drawItemsByCategory(doc, y, dim, data.claims, data.layout.accent_color)
    : drawItemsByDate(doc, y, dim, data.claims, data.layout.accent_color)
  if (data.layout.show_declaration) drawDeclaration(doc, y, dim, data, signatureDataUrl)
  if (data.layout.show_receipt_appendix) await drawReceiptAppendix(doc, dim, data, supabase, data.layout.accent_color)
  if (data.layout.show_tng_appendix && data.tng_statements.length > 0) drawTngAppendixCover(doc, dim, data.tng_statements, data.layout.accent_color)
}

function drawHeader(doc: PDFKit.PDFDocument, y: number, dim: PageDimensions, accent: string): number {
  const H = 44
...
function drawClaimantInfo(doc: PDFKit.PDFDocument, y: number, dim: PageDimensions, data: PdfData): number {
  const H = 84
  const HALF = ML + dim.CW / 2
  const meta = [data.claimer_department, data.claimer_location].filter(Boolean).join(' · ')
  const company = data.claimer_company_name || data.org_name
  ...
  if (meta) doc.text(meta, ML + 12, y + 50, { lineBreak: false, width: dim.CW / 2 - 24 })
  doc.text(company, ML + 12, y + 64, { lineBreak: false, width: dim.CW / 2 - 24 })
...
function drawSummaryTable(doc: PDFKit.PDFDocument, y: number, dim: PageDimensions, claims: PdfClaim[], accent: string): number {
  ...
  for (let i = 0; i < claims.length; i++) {
    ...
    const c = claims[i]
    drawTblRow(doc, y, cols, [c.title, c.period, c.status, String(c.items.length), c.total_myr.toFixed(2)], i % 2 === 0 ? C.white : C.light, { status: statusCol(c.status) })
    y += ROW_H
  }
  ...
  doc.text('MYR ' + grand.toFixed(2), amtX + 4, y + 6, { width: lastCol.w - 8, align: 'right', lineBreak: false })
  return y + ROW_H + 2 + 24
}

function drawItemsByDate(doc: PDFKit.PDFDocument, y: number, dim: PageDimensions, claims: PdfClaim[], accent: string): number {
  ...
  for (const claim of claims) {
    ...
    const statusStr = claim.status === 'SUBMITTED' ? `Submitted: ${claim.submitted_at}` : 'DRAFT — not yet submitted'
    ...
    doc.font(BOLD).fontSize(8.5).fillColor(C.dark)
    doc.text('MYR ' + claim.total_myr.toFixed(2), subtX + 4, y + 5, { width: cols[cols.length - 1].w - 8, align: 'right', lineBreak: false })
    y += 38
  }
...
function drawItemsByCategory(doc: PDFKit.PDFDocument, y: number, dim: PageDimensions, claims: PdfClaim[], accent: string): number {
  ...
  const renderCat = (label: string, headerBg: string, headerFg: string, items: PdfItem[]) => {
    ...
    const subtX = ML + cols.slice(0, -1).reduce((s, c) => s + c.w, 0)
    doc.font(BOLD).fontSize(8.5).fillColor(C.dark)
    doc.text('MYR ' + catTotal.toFixed(2), subtX + 4, y + 5, { width: cols[cols.length - 1].w - 8, align: 'right', lineBreak: false })
    y += 24
  }

  for (const cat of CATEGORY_ORDER) {
    const items = typeMap.get(cat.type)
    if (items?.length) renderCat(cat.label, cat.headerBg, cat.headerFg, items)
  }
  for (const [type, items] of typeMap.entries()) {
    if (!knownTypes.has(type) && items.length) renderCat(type, accent, C.white, items)
  }
  return y
}

function drawDeclaration(doc: PDFKit.PDFDocument, y: number, dim: PageDimensions, data: PdfData, signatureDataUrl: string | null): void {
  ...
  const declarationText = 'I hereby certify that the expenses claimed in this document were incurred wholly, exclusively and necessarily in the course and performance of my official duties. All amounts stated are true and accurate to the best of my knowledge, and where receipts are attached they are genuine documents. I understand that submitting a false or inaccurate claim may result in disciplinary action.'
  ...
  doc.moveTo(ML + 10, y + SIG_H - 14).lineTo(ML + SIG_W - 10, y + SIG_H - 14).lineWidth(0.5).stroke(C.border)

  const RX = ML + SIG_W + 20
  let iy = y + 8
  for (const [lbl, val] of [
    ['FULL NAME', data.claimer_name],
    ['COMPANY', data.claimer_company_name || data.org_name],
  ...
  doc.text('Generated by myexpensio · System-generated · audit-ready', ML, footY, { width: dim.CW, align: 'center', lineBreak: false })
}

async function drawReceiptAppendix(doc: PDFKit.PDFDocument, dim: PageDimensions, data: PdfData, supabase: SupabaseClient, accent: string): Promise<void> {
  const receiptItems: Array<{ label: string; path: string }> = []
  for (const claim of data.claims) {
    for (const item of claim.items) {
      if (item.receipt_path) {
        receiptItems.push({ label: `${claim.title}  ·  ${item.date}  ·  ${item.type}  ·  ${item.description}`.slice(0, 90), path: item.receipt_path })
      }
    }
  }
...
function drawTngAppendixCover(doc: PDFKit.PDFDocument, dim: PageDimensions, tngStatements: TngStatement[], accent: string): void {
  doc.addPage()
  let y = MT
  doc.rect(ML, y, dim.CW, 44).fill(accent)
  doc.font(BOLD).fontSize(13).fillColor(C.white)
...
async function mergeTngStatements(mainBuf: Buffer, tngStatements: TngStatement[], supabase: SupabaseClient): Promise<Buffer> {
  if (tngStatements.length === 0) return mainBuf
...
function drawTblHeader(doc: PDFKit.PDFDocument, y: number, cols: ColDef[], accent: string): number {
