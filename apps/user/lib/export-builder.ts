...
import { PRESET_COLUMNS, getColumnLabel, type ExportColumnKey } from './export-columns'
...
function getCellValue(key: ExportColumnKey, claim: ClaimForExport, item: ItemForExport): string | number {
  switch (key) {
    ...
    case 'mileage_rate': return item.type === 'MILEAGE' ? (item.rate ?? '') : ''
...
function flattenToRows(claims: ClaimForExport[], columns: ExportColumnKey[]): (string | number)[][] {
  const rows: (string | number)[][] = []
  for (const claim of claims) {
    if (claim.items.length === 0) {
      rows.push(columns.map((k) => getCellValue(k, claim, {} as ItemForExport)))
    } else {
      for (const item of claim.items) {
        rows.push(columns.map((k) => getCellValue(k, claim, item)))
      }
    }
  }
  return rows
...
export function buildCSV(claims: ClaimForExport[], options: BuildExportOptions = {}): string {
  const columns = options.columns?.length ? options.columns : PRESET_COLUMNS.STANDARD
  const headers = columns.map(getColumnLabel)
  const rows = flattenToRows(claims, columns)
...
export async function buildXLSX(claims: ClaimForExport[], options: BuildExportOptions = {}): Promise<Buffer> {
  const columns = options.columns?.length ? options.columns : PRESET_COLUMNS.STANDARD
  const headers = columns.map(getColumnLabel)
  const rows = flattenToRows(claims, columns)
  ...
  const sheet = workbook.addWorksheet('Claims', { pageSetup: { paperSize: 9, orientation: 'landscape' } })
...
export async function buildExport(
  ...
  switch (format) {
    case 'CSV':
      return { buffer: Buffer.from(buildCSV(claims, options), 'utf-8'), contentType: 'text/csv; charset=utf-8', extension: 'csv' }
    case 'XLSX': {
      const buf = await buildXLSX(claims, options)
      return { buffer: buf, contentType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', extension: 'xlsx' }
    }
    case 'PDF':
...
export async function buildExportRows(
  ...
  for (const claim of claims) {
    const claimItems = Array.isArray(claim.claim_items) ? claim.claim_items : [claim.claim_items].filter(Boolean)
    for (const item of claimItems) {
      ...
      rows.push({
        ...
        mileage_rate: item.type === 'MILEAGE' && item.rate != null ? String(item.rate) : '',
