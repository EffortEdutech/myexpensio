...
- [x] New dependency: **`expo-audio`** (Expo's current, non-deprecated recording API — `expo-av`'s Audio module is deprecated). Approved by Eff 2026-07-18 and installed at the Expo SDK-compatible `~56.0.12`. `app.json` has the config plugin entry + `NSMicrophoneUsageDescription` (iOS) + `RECORD_AUDIO` (Android).
...
**Verification update (2026-07-19):** `expo-audio@56.0.12` is installed and `apps/user-mobile-v2` passes `tsc --noEmit` against its real declarations. The connected Samsung test phone is visible through ADB, and `pnpm logcat:expo` now streams process-scoped Expo Go logs from the repo root. The remaining S4 gate is manual end-to-end device QA (record → finalize audio → Gemini parse → type confirmation → review modal), not dependency installation or compilation.

### Testing checklist
- [x] `expo-audio@56.0.12` installed and `pnpm --filter @myexpensio/user-mobile-v2 typecheck` clean (verified 2026-07-19)
