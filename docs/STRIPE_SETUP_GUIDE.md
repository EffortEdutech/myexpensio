...
- Customers can subscribe to the **Pro plan** (monthly or yearly)
...
You will now create the Pro plan products in Stripe's **test mode** first. Test mode uses fake money — nothing real is charged.

### Step 4 — Create the Pro Monthly Price

In your Command Prompt, run this command. Replace `2900` with your actual monthly price **in cents** (e.g. RM 29.00 = `2900`, USD $19.00 = `1900`):

```
stripe prices create --unit-amount=2900 --currency=myr --recurring[interval]=month --product-data[name]="myexpensio Pro" --nickname="Pro Monthly"
```
...
**Copy the `id` value** (starts with `price_`). This is your **STRIPE_PRO_MONTHLY_PRICE_ID**.

> Change `--currency=myr` to `--currency=usd` or whichever currency your business uses.

---

### Step 5 — Create the Pro Yearly Price

Run this command. Replace `29000` with your yearly price in cents (e.g. RM 290.00 = `29000`):

```
stripe prices create --unit-amount=29000 --currency=myr --recurring[interval]=year --product-data[name]="myexpensio Pro" --nickname="Pro Yearly"
```

**Copy the `id` value** from the output. This is your **STRIPE_PRO_YEARLY_PRICE_ID**.
...
Webhooks are how Stripe tells your app "someone just paid" or "a subscription was cancelled." This is the most important part.
...
Click **Add New** for each variable below. Set **Environment** to **Production** (and optionally **Preview**).

| Variable Name | Value | Where to get it |
|---|---|---|
| `STRIPE_SECRET_KEY` | `sk_test_...` | Stripe Dashboard → Developers → API keys |
| `STRIPE_WEBHOOK_SECRET` | `whsec_...` | Step 8 above |
| `STRIPE_PRO_MONTHLY_PRICE_ID` | `price_...` | Output from Step 4 |
| `STRIPE_PRO_YEARLY_PRICE_ID` | `price_...` | Output from Step 5 |
| `STRIPE_PRO_MONTHLY_AMOUNT` | `29` | Your monthly price in whole number (e.g. 29 for RM 29) |
| `STRIPE_PRO_YEARLY_AMOUNT` | `290` | Your yearly price in whole number (e.g. 290 for RM 290) |

> **Important:** `STRIPE_PRO_MONTHLY_AMOUNT` and `STRIPE_PRO_YEARLY_AMOUNT` are the display amounts shown in the UI — use whole numbers (29, not 2900 and not 29.00).
...
1. Open your user app in a browser
2. Log in as a test user
3. Go to the Billing page and click **Upgrade to Pro**
...
4. Wait for Stripe to approve your account (usually same day to 2 days)

---

### Step 15 — Create Live Products and Prices

Switch to **Live mode** in the Stripe Dashboard (toggle at the top), then repeat Steps 4 and 5 but now in your Command Prompt first run:

```
stripe login
```

Then create the prices the same way — the commands are identical. You will get new `price_live_...` IDs.

Alternatively, do it directly in the Stripe Dashboard:
1. Go to **Products** → **Add product**
2. Create "myexpensio Pro" with monthly and yearly prices
...
| `STRIPE_PRO_MONTHLY_PRICE_ID` | Live monthly price ID |
| `STRIPE_PRO_YEARLY_PRICE_ID` | Live yearly price ID |

Then **redeploy** again (Step 11).

---

## Quick Reference — All 6 Environment Variables
...
| `STRIPE_PRO_MONTHLY_PRICE_ID` | `price_1ABC...` | `price_1XYZ...` |
| `STRIPE_PRO_YEARLY_PRICE_ID` | `price_1DEF...` | `price_1UVW...` |
| `STRIPE_PRO_MONTHLY_AMOUNT` | `29` | `29` (same) |
| `STRIPE_PRO_YEARLY_AMOUNT` | `290` | `290` (same) |
...
→ The price IDs in Vercel do not match what Stripe has. Re-copy the price IDs from Step 4/5 and update the variables.

---

*Guide written for myexpensio — last updated May 2026*
